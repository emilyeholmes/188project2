...
def sentence1() -> Expr:
    ...
    one = Expr('A') | Expr('B')
    two = ~Expr('A') % (~Expr('B') | Expr('C'))
    three = disjoin([~Expr('A'), ~Expr('B'), Expr('C')])
...
def sentence2() -> Expr:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    one = Expr('C') % (Expr('B') | Expr('D'))
    two = Expr('A') >> (~Expr('B') & ~Expr('D'))
    three = ~(Expr('B') & ~Expr('C')) >> Expr('A')
    four = ~Expr('D') >> Expr('C')
...
def entails(premise: Expr, conclusion: Expr) -> bool:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    # not(a implies b) is false in all worlds
    # a & not b is false in all worlds
...
def foodLogicPlan(problem) -> List:
    ...
    initialFood = [PropSymbolExpr(food_str, x, y, time=0) for x,y, in food]
    ...
    for t in range(50):
        ...
        foodAtSquare = [foodSuccessorAxiom(x,y,t) for x,y in food]
        for i in foodAtSquare:
            KB.append(i)
        ...
        pacmanMadeMove = [PropSymbolExpr(action, time=t) for action in DIRECTIONS]
        KB.append(exactlyOne(pacmanMadeMove))
        lst = [pacmanSuccessorAxiomSingle(x, y, t+1, walls) for x,y in non_wall_coords]
        for i in lst:
            KB.append(i)
    "*** END YOUR CODE HERE ***"
...
def localization(problem, agent) -> Generator:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    # util.raiseNotDefined()
    walls = [PropSymbolExpr(wall_str, x, y) for x,y in walls_list]
    not_walls = [~PropSymbolExpr(wall_str, x, y) for x,y in all_coords if (x,y) not in walls_list]
    KB.append(conjoin(walls))
